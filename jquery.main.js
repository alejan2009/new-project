...
function loginBar(){
	var loginBar = $('.login');
	var opener = $('.login-opener');
	var animSpeed = 500;
	loginBar.hide();
	opener.click(function(){
		loginBar.slideDown(animSpeed);
		opener.parent('li').addClass('active');
		return false;
	})
}
...
function initIsotope(){
	...
	var visualItems = visual.find('.box');
	var animSpeed = 500;
	var loginBar = $('.login');
	var visualOverlay = visualItems.find('.overlay');
	var contentOpener = $('.content-opener');
	var linksList = $('#header .col-2 ul li');
	visualItems.mouseenter(function(){
		var currentVisualItem = $(this);
		var currentOverlay = currentVisualItem.find('.overlay');
		currentOverlay.stop(true,true).fadeIn(animSpeed);
	}).mouseleave(function(){
		var currentVisualItem = $(this);
		var currentOverlay = currentVisualItem.find('.overlay');
		currentOverlay.stop(true,true).fadeOut(animSpeed);
	})
	var openerLinks = visualItems.find('.view');
	var data,
		path,
		currentItem,
		currentLink;
	var currentIndex = 0;
	var articlePrev = $('.popup .article-nav .prev a');
	var articleNext = $('.popup .article-nav .next a');
	var closeLink = $('.popup .close');
	...
	function loadContent(){
		$.ajax({
			...
			dataType:'html',
			beforeSend:function(){
				currentItem.find('.img-holder').append(loader);
			},
			success:function(response){
				loader.remove();
				data = $(response);
				data.insertAfter(currentItem);
				visual.isotope('reloadItems').isotope({sortBy:'original-order'},function(){
					var currentPopupPosition = parseInt($('.popup').offset().top);
					console.log()
					$.scrollTo(currentPopupPosition, animSpeed);
				});
				fader.appendTo(visual);
			},
			error:function(){
	...
	openerLinks.click(function(e){
		currentLink = $(this);
		currentIndex = openerLinks.index(currentLink);
		currentItem = currentLink.closest('.box');
		path = currentLink.attr('href');
		loadContent();
		return false;
	});
	contentOpener.click(function(e){
		...
		if(loginBar.is(':visible')){
			loginBar.slideUp(animSpeed);
		}
		return false;
	});
	articlePrev.live('click',function(){
		if(currentIndex > 0){
			currentIndex--;
			currentLink = openerLinks.eq(currentIndex);
			currentItem = currentLink.closest('.box');
			path = currentLink.attr('href');
			closePopup();
			setTimeout(loadContent,900)
	...
	articleNext.live('click',function(){
